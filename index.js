...
module.exports = cli => {
  return cli
    ...
    .option('--adbPort [port]', 'Port for connect to a device on a remote host. Use with --adbHost', 0)
    .option('--marionetteHost [host]', 'Connect to Marionette on a remote host. Tip: use with --marionettePort', 'localhost')
    .option('--marionettePort [port]', 'Port for connect to Marionette on a remote host. Use with --marionetteHost', 2828)
