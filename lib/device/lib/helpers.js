...
var PATTERNS = {
  ...
  ddmsSave: xRegExp('^(?<timestamp>\\d\\d-\\d\\d\\s\\d\\d:\\d\\d:\\d\\d\\.\\d+):*\\s(?<level>VERBOSE|DEBUG|ERROR|WARN|INFO|ASSERT)\\/(?<tag>.*?)\\((?<pid>\\s*\\d+)\\):\\s(?<message>.*)$')
};
...
Helpers.gaiaRevision = null;
Helpers.geckoRevision = null;
...
Helpers.prototype.getGeckoRevision = function() {
  if (Helpers.geckoRevision) {
    return Promise.resolve(Helpers.geckoRevision);
  }
...
Helpers.prototype.getGaiaRevision = function() {
  if (Helpers.gaiaRevision) {
    return Promise.resolve(Helpers.gaiaRevision);
  }
...
Helpers.prototype.parseLog = function(contents) {
  ...
  contents
    ...
    .forEach(function(line) {
      ...
      var lineStripped = line.replace(/\s+$/g);

      if (!type) {
        type = getLogMessageType(lineStripped);
      }

      if (!type || !lineStripped.length) {
        return;
      }
      ...
      try {
        var match = xRegExp.exec(lineStripped, regex);
