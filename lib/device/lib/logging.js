...
/**
 * Write the filesize information to the log for a given file
 * @param {string} on-device path and filename to retrieve filesize for
 * @returns {Promise}
 */
Logging.prototype.filesize = function(remoteFile, context) {
  var logging = this;
  var command = this.device.command()
    .env('ANDROID_SERIAL', this.serial)
    .adbShell('ls -l ' + remoteFile);

  return command
    .exec()
    .then(function(output) {
      var parts = output.split(/\s+/g);
      logging.info('PerformanceFilesize', context + '|' + remoteFile + '|' + parts[3]);
    });
};
