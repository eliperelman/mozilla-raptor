...
Logging.prototype.write = function(priority, tag, message) {
  ...
  var command = util.format('log -p %s -t %s "%s"', priority, tagCmd, messageCmd);
...
Logging.prototype.memory = function(pid, context) {
  ...
  return command
    ...
    .then(function(output) {
      ...
      var index = parts.indexOf(pidStr) + 1;
...
Logging.prototype.start = function() {
  ...
  var env = process.env; // eslint-disable-line
