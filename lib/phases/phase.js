...
Phase.FILESIZEENTRY = 'filesizeentry';
...
/**
 * Wait for the filesize
 * @returns {Promise}
 */
Phase.prototype.waitForFilesize = function() {
  var phase = this;
  var hasFilesize = false;
  return new Promise(function(resolve) {
    phase.dispatcher.on(Phase.FILESIZEENTRY, function handler(entry) {
      phase.dispatcher.removeListener(Phase.FILESIZEENTRY, handler);
      resolve(entry);
    });
  });
}
