...
var mozTable = require('markdown-table');
...
var Phase = function(options) {
  ...
  this.log('@mozilla/raptor v%s: Preparing to start testing...', this.options.version);
  this.resetTimeout();
};
...
Phase.prototype.getDevice = function() {
  ...
  return mozDevice(this.options.serial, this.options)
...
Phase.prototype.annotate = function() {
  ...
  var gaia = this.createAnnotationPoint('Gaia', revisions.gaia);
  var gecko = this.createAnnotationPoint('Gecko', revisions.gecko);
...
Phase.prototype.logStats = function(statistics) {
  if (this.options.output === 'json') {
    console.log(JSON.stringify(statistics)); // eslint-disable-line
    return;
  }
