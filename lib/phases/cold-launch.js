...
ColdLaunch.prototype.capture = function() {
  ...
  this.dispatcher.on(Phase.PERFORMANCEENTRY, function(entry) {
    ...
    var ignore = phase.fullyLoaded &&
        perfEntry.entryType === 'mark' &&
        perfEntry.name !== phase.START_MARK;

    if (ignore) {
      return;
    }

    if (perfEntry.context !== app) {
      return;
    }

    if (perfEntry.name === 'fullyLoaded') {
      phase.fullyLoaded = true;
    }

    if (entryPoint) {
      perfEntry.entryPoint = entryPoint;
    }

    phase.results.push(perfEntry);
  });
  ...
  this.dispatcher.on(Phase.MEMORYENTRY, function(entry) {
    ...
    if (memEntry.context !== app) {
      return;
    }

    if (entryPoint) {
      memEntry.entryPoint = entryPoint;
    }

    phase.results.push(memEntry);
  });
};
