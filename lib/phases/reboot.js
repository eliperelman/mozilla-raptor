...
var filesizeParser = require('../parsers/filesize');
...
Reboot.prototype.setup = function() {
  ...
  if (this.options.remoteFile) {
    this.registerParser(filesizeParser);
  }
  this.capture();
};
...
Reboot.prototype.capture = function() {
  ...
  /**
   * Captured filesize entries: All
   */
  this.dispatcher.on(Phase.FILESIZEENTRY, function(entry) {
    phase.debugEventEntry(Phase.FILESIZEENTRY, entry);
    phase.setEntryAppName(entry);
    phase.results.push(entry);
  });
