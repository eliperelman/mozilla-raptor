...
Device.prototype.logstream = function() {
  debug(`Initializing log stream`);

  return new Promise((resolve) => {
    let request = http.get(`${this.url}/logs`, (response) => {
      let stream = byline.createStream();

      capture(stream);
      resolve([response.pipe(stream), request]);
    });
  });
};

Device.prototype.filestream = function(stream, destination) {
  return stream.pipe(fs.createWriteStream(destination, { flags: 'a' }));
};
